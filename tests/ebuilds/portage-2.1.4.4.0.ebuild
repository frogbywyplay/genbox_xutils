...
# $Header: /var/cvsroot/gentoo-x86/sys-apps/portage/portage-2.1.4.4.ebuild,v 1.9 2008/06/01 10:30:18 zmedico Exp $

inherit toolchain-funcs eutils flag-o-matic multilib mercurial-mq
...
EHG_REPO_URI="genbox/$PN"
EHG_BRANCH="sys-apps/portage-2.1.4.4"

EHG_MQ_REPO_URI="genbox/$PN/.hg/patches"
EHG_MQ_BRANCH="sys-apps/$PN-${PV%\.*}"
EHG_MQ_GUARDS="stable"
...
pkg_postinst() {
	...
	if ! $(grep -q "^PORTDIR" "${ROOT}/etc/make.conf"); then
		ewarn "PORTDIR not found in $ROOT/etc/make.conf"
		ewarn "Adding /usr/portage as PORTDIR"
